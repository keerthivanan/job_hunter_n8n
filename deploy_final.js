...
function loadEnv() {
  ...
  } catch (e) { console.log('.env file not found, using process.env'); }
}
loadEnv();
...
if (!KEY || !OPENAI_KEY || !HOST || !WORKFLOW_ID || !CV_PATH) {
...
// ── GENERATE RESUME: actual CV PDF ───────────────────────────────────
const GENERATE_RESUME_CODE = `const j = $input.item.json;
const CV_BASE64 = '${CV_BASE64}';
return { json: { ...j, resumeBase64: CV_BASE64 } };`;

// ── PREPARE PDF BINARY: use prepareBinaryData for proper attachment ───
const PREPARE_PDF_CODE = `const j = $input.item.json;
...
// ── FILTER: strict dedup — NO reprocessing seen jobs ─────────────────
const FILTER_CODE = `let normalizedItems;
...
// ── COMPOSE EMAIL: short, sharp OpenAI email (120 words max) ─────────
const COMPOSE_EMAIL_CODE = `const j = $input.item.json;
const creds = await this.getCredentials('openAiApi');
const OPENAI_KEY = creds.apiKey;
const targetEmail = 'keerthivanan7@gmail.com'; // TEST MODE
...
// ── FIND HR EMAIL ─────────────────────────────────────────────────────
const FIND_HR_EMAIL_CODE = `const j = $input.item.json;
...
async function main() {
  ...
  console.log('All nodes patched (FINAL version).');

  fs.writeFileSync('APPLY_NOSCORE_FINAL.json', JSON.stringify(wf, null, 2));

  console.log('Deactivating...'); await api('POST', '/api/v1/workflows/' + WORKFLOW_ID + '/deactivate', null); await sleep(2000);
  ...
  const n2 = JSON.parse(JSON.stringify(push.d.nodes)), c2 = JSON.parse(JSON.stringify(push.d.connections));
  const t = n2.find(n => n.name === 'Run Manually');
  const wp = 'apply-f-' + Date.now();
  ...
  await api('PUT', '/api/v1/workflows/' + WORKFLOW_ID, { name:wf.name, nodes:n2, connections:c2, settings:{executionOrder:'v1',saveDataSuccessExecution:'all',saveDataErrorExecution:'all'} });
  const act = await api('POST', '/api/v1/workflows/' + WORKFLOW_ID + '/activate', null);
  if(act.s!==200){console.log('Activate FAILED:',JSON.stringify(act.d).slice(0,200));return;}
  console.log('Activated. Waiting 20s...');
  await sleep(20000);

  const wh = await httpGet('/webhook/'+wp);
  console.log('Webhook:', wh.s, wh.d.slice(0,80));
  if(wh.s!==200){await restore(wf);return;}

  let execId=null;
  console.log('Watching (up to 10 min)...');
  for(let i=0;i<120;i++){
    await sleep(5000);
    ...
    if(nx){
      ...
      if(nx.status!=='running'&&nx.status!=='waiting'&&nx.status!=='new'){console.log('Finished: '+nx.status);break;}
    }
  ...
  console.log('\nDONE! Check keerthivanan7@gmail.com — short pro email + PDF attachment!');
}

async function restore(wf){
  await api('POST','/api/v1/workflows/' + WORKFLOW_ID + '/deactivate',null);
  await api('PUT','/api/v1/workflows/' + WORKFLOW_ID,{name:wf.name,nodes:wf.nodes,connections:wf.connections,settings:{executionOrder:'v1',saveDataSuccessExecution:'all',saveDataErrorExecution:'all'}});
